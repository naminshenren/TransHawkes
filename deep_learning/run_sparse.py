...
tf.flags.DEFINE_float("l2", 0.5, "l2.")
...
tf.flags.DEFINE_float("emb_learning_rate", 0.0005, "embedding learning_rate.")
tf.flags.DEFINE_float("dropout_prob", 0.8, "dropout probability.")
...
#batch_x, batch_x_indict, batch_y, batch_time_interval_index, batch_rnn_index = get_batch(x_train, y_train, sz_train, time_train,rnn_index_train,config.n_time_interval,1, batch_size=batch_size)
sess = tf.Session(config = tf.ConfigProto(allow_soft_placement = True))
start = time.time()
model = SDPP(config, sess, n_nodes, hp)
sess.graph.finalize()
step = 0
best_val_loss = 1000
best_test_loss = 1000
...
while step * batch_size < training_iters:
    ...
    batch_rnn_index = np.array(batch_rnn_index)
    #print("batch x",len(batch_x),len(batch_x[0]))
    lambda1 = model.train_batch(batch_x[:,0:16],batch_x_indict, batch_y,batch_time_interval_index,batch_rnn_index[:,0:16],batch_x[:,1:17])
    #train_writer.add_summary(summary, step)
    train_loss.append(model.get_error(batch_x,batch_x_indict, batch_y,batch_time_interval_index,batch_rnn_index,batch_ys))
	...
	# print lambda1
        ...
        for val_step in range(len(y_val)/batch_size):
            ...
            val_rnn_index = np.array(val_rnn_index)
            val_loss.append(model.get_error(val_x[:,0:16], val_x_indict, val_y, val_time_interval_index,val_rnn_index[:,0:16],val_x[:,1:17]))
        test_loss = []
        for test_step in range(len(y_test)/batch_size):
            ...
            test_rnn_index = np.array(test_rnn_index)
            test_loss.append(model.get_error(test_x[:,0:16],test_x_indict, test_y, test_time_interval_index,test_rnn_index[:,0:16],test_x[:,1:17]))
        ...
        for test_step in range(len(y_test) / batch_size+1):
            ...
            test_x = np.array(test_x)
            test_rnn_index = np.array(test_rnn_index)
            predict_result.extend(model.predict(test_x[:,0:16], test_x_indict, test_y, test_time_interval_index, test_rnn_index[:,0:16],test_x[:,1:17]))
            test_loss.append(model.get_error(test_x, test_x_indict, test_y, test_time_interval_index, test_rnn_index,test_ys))
