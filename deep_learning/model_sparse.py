...
class SDPP(object):
    def __init__(self, config, sess, n_nodes, hp):
        ...
        self.transformers = Transformer(hp)
    ...
    def build_input(self):
        self.x = tf.placeholder(tf.int32, shape=[None, self.n_steps-1], name="x")
        # (total_number of sequence,n_steps)
        self.ys = tf.placeholder(tf.int32, shape=[None, self.n_steps-1], name="ys")
        ...
        self.rnn_index = tf.placeholder(tf.float32, [None,self.n_steps-1], name="rnn_index")
        # (total_number of sequence,n_steps)
    ...
    def build_model(self):
        with tf.device('/gpu:0'):
            with tf.variable_scope('deephawkes') as scope:
                ...
                with tf.variable_scope('transformerhawkes'):
                    src_masks = self.rnn_index#tf.zeros_like(self.x)
                    enc = tf.nn.dropout(tf.nn.embedding_lookup(self.embedding, self.x),
                                             self.dropout_prob)
                    dec = tf.nn.dropout(tf.nn.embedding_lookup(self.embedding, self.ys),
                                             self.dropout_prob)
                    print("Decc: ", dec.get_shape())
                    memory= self.transformers.encode(self.x,enc,src_masks)
                    logits = self.transformers.decode(self.ys, memory, dec, src_masks)
                    hidden_states = tf.reduce_sum(logits, reduction_indices=[1])
                ...
                with tf.variable_scope('SumPooling'):
                    ...
                    hidden_graph = tf.SparseTensor(indices = self.x_indict, values=hidden_graph_value,
                                                   dense_shape=[self.batch_size, self.n_sequences, 2 * self.n_hidden_gru])
    ...
    def train_batch(self, x, x_indict,y, time_interval_index,rnn_index,ys):
        ...
        # print rnn_state
        return time_weight
    def get_embedding(self, x, x_indict,y, time_interval_index,rnn_index,ys):
        ...
        # print rnn_state
        return embedding
    def get_error(self, x, x_indict,y, time_interval_index,rnn_index,ys):
        return self.sess.run(self.error, feed_dict={self.x: x, self.x_indict: x_indict, self.y: y,
                                                                     self.time_interval_index:time_interval_index,
                                                                     self.rnn_index:rnn_index,self.ys:ys})
    def predict(self, x, x_indict,y, time_interval_index,rnn_index,ys):
